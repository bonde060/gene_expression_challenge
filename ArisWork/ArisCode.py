...
def save_predicted_interactions(predictions, geneNames, wholeLine, file_name="predicted_interactions.csv"):
    # Save the predicted interactions to an Excel file
    predicted_interactions_df = pd.DataFrame(predictions, columns=wholeLine, index=geneNames).T
    predicted_interactions_df.to_csv(file_name, index=True)
    
def format_test_data(data):
    fp = open("ChallengeB_release_hold_out_40_query_genes.txt", "r")
    holdOutGenes = fp.readlines()
    fp.close()
    holdOutGenes.pop(0)
    print(holdOutGenes)
    for i in range(len(holdOutGenes)):
        holdOutGenes[i] = holdOutGenes[i][:-1]
    X_test = data.loc[holdOutGenes]
    return X_test
...
if __name__ == "__main__":
    ...
    wholeLine = data.index
    # cols = X.index
    X_scaled = prepare_data(data, X)
    model = create_model(X_scaled.shape[1])
    
    X_test = format_test_data(data)
    geneNames = X_test.index
    X_test_scaled = prepare_data(data, X_test)
    ...
    predicted_interactions = model.predict(X_test_scaled)
    #corr, pval = pearsonr(predicted_interactions[0], Y.iloc[0])

    #print(corr)
    #print(type(predicted_interactions))

    # Save the predicted interactions to an Excel file
    save_predicted_interactions(predicted_interactions, geneNames, wholeLine, "all_predicted_interactions.csv")
